...
def plot_linear_fit(x, y, x_err, y_err, slope, intercept, slope_err, intercept_err, r_value, x_label="Eje X", y_label="Eje Y", title="Ajuste lineal con barras de error"):
    ...
    textstr = f"$r = {r_value:.2f}$\n$\sigma(A) = {slope_err:.5f}$\n$\sigma(B) = {intercept_err:.5f}$"
...
if __name__ == "__main__":
    ...
    title = st.text_input("Título del gráfico", "Ajuste lineal con barras de error")
    ...
    if st.button("Ejecutar regresión lineal"):
        try:
            ...
            plot_linear_fit(x_data, y_data, x_err, y_err, slope, intercept, slope_err, intercept_err, r_value, x_label, y_label, title)
